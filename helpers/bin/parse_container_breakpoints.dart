...
void main(List<String> args) {
  ...
  File outFile = File("lib/src/tailwind_breakpoint_container.dart");
  ...
  ContainerBreakpointVisitor visitor = ContainerBreakpointVisitor();
  tree.visit(visitor);

  // print(tree.toDebugString());
  print(visitor.parsedBreakpoints);

  LibraryBuilder lib = LibraryBuilder();
  // lib.directives.add(Directive.import("dart:ui"));

  Class radiusClass = Class((cb) {
    cb.name = "TwContainerBreakpoint";
    cb.abstract = true;
    cb.docs.add("/// All container width breakpoints defined in Tailwind CSS");

    for (var el in visitor.parsedBreakpoints.entries) {
      cb.fields.add(
        Field((f) {
          f.name = el.key;
          f.type = Reference("double");
          f.static = true;
          f.modifier = FieldModifier.constant;
          f.assignment = Code(el.value.toString());
        }),
      );
    }
